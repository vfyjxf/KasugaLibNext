...
@Getter
public class MaterialAmbientMorph<IdType> implements MorphType<Material, Vector4f, IdType> {

    private final Material original;
    private final IdType identifier;
    private final Vector4f targetAmbient;

    public MaterialAmbientMorph(Material original, IdType identifier, Vector4f targetAmbient) {
    ...
    public Vector4f morph(Material input, float percentage, float factor) {
        ...
        return new Vector4f(
                targetAmbient.x() * weight,
                targetAmbient.y() * weight,
                targetAmbient.z() * weight,
                1f
        );
    }
}

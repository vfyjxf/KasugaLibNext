...
public class GraphSimplifier {
    ...
    public static SimplifiedSubGraph simplify(TrackGraph graph){
        TrackEdge beginEdge = findTrackWithoutBezier(graph);
        if(beginEdge == null)
            return new SimplifiedSubGraph();
        List<Couple<TrackNode>> frontier = new ArrayList<>();
        frontier.add(Couple.create(beginEdge.node1, null));
        return simplify(graph, frontier, (edge)->true);
    }

    public static SimplifiedSubGraph simplify(
            TrackGraph graph,

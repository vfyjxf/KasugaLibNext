...
import lib.kasuga.create.content.train.graph.*;
import lib.kasuga.create.mixins.TrainMixin;
...
public class CustomTrackSegmentPropagator{
    ...
    public void propagate(TrackGraph graph, CustomBoundary boundary, boolean direction){
        ...
        RailwayData data = railwayManager.getData();

        GraphExtraData extraData = data.withGraph(graph);
        ...
        walk(
                ...
                (val)->{
                    ...
                    if(data.hasSegment(featureName, currentGroup)){
                        data.removeSegment(featureName, currentGroup);
                    }
                ...
                (val)->{
                    if(val.hasBoundaryFeature(featureName) && val.getBoundaryFeature(featureName) != EdgeExtraData.passiveBoundaryGroup){
                        data.removeSegment(featureName, val.getBoundaryFeature(featureName));
                    }
                    // TrackEdgeLocation location = extraData.findLocationOf(val);
                    val.setBoundaryFeature(featureName, segmentId);
                    return true;
                },
                (immediateBoundary)->{
                    immediateBoundary.setSegment(!direction, segmentId);
                },
                false
        );
        data.addSegment(featureName, segmentId, segment);
    }

    public void notifyNewNode(TrackGraph graph, TrackNode node){
    ...
    public void onRemoved(TrackGraph graph, CustomBoundary customBoundary) {
            ...
            walk(
                    ...
                    boundaryPair->{
                        ...
                        if(!edgeData.hasCustomBoundaryInThisEdge(featureName)){
                            data.removeSegment(featureName, edgeData.getBoundaryFeature(featureName));
                    ...
                    (immediateBoundary)->{
                        immediateBoundary.markDirty(!front);
                    },
                    false
            );
        }
    ...
    public void walk(
        ...
        if (!forCollection) {
            ...
            notifyTrains(featureName, graph, startEdge, oppositeEdge);
        }
    ...
    private void walk(
        ...
        GraphExtraData graphExtraData = railwayManager.getData().withGraph(graph);
        ...
        while (!frontier.isEmpty()) {
            ...
            EdgeWalk: for (Map.Entry<TrackNode, TrackEdge> entry : graph.getConnectionsFrom(currentNode)
                ...
                for (boolean flip : Iterate.falseAndTrue) {
                    ...
                    EdgeExtraData extraData = graphExtraData.getOrComputeEdgeData(currentEdge);
                    ...
                    if (!extraData.hasCustomBoundaryInThisEdge(featureName)) {
                        if (nonBoundaryCallback.test(extraData)) {
                            notifyTrains(featureName, graph, currentEdge);
                    ...
                    if (circuitCallback.test(Pair.of(currentNode, nextBoundary))) {
                        notifyTrains(featureName, graph, edge, oppositeEdge);
    ...
    public static void notifyTrains(ResourceLocation segmentType, TrackGraph graph, TrackEdge... edges) {
            ...
            for (Train train : Create.RAILWAYS.trains.values()) {
                ...
                train.forEachTravellingPoint(tp -> {
                    if (tp.edge == trackEdge){
                        ((TrainDeviceProvider) train).kasugaLib$getDeviceManager().onSegmentUpdated(segmentType);
                    }
                });
            }
        }
